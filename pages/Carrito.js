...
export default function Carrito() {
    ...
    useEffect(() => {
        ...
        const getData2 = async () => {
            try {
                ...
                const SumaNueva = jsonValue.length;
    ...
    const deleteArticle = (item) => {
        ...
        if (newCarrito.length > 0) {
            ...
            calTotal(newCarrito);
            setCarritoState(carritoState - 1)
        } else {
            setCarrito(null);
            setCarritoState()
        ...
        if (carrito && carrito !==0 ) {
                    ...
                    <ScrollView>
                        {
                            carrito.map((item, index) => {
                                //  console.log("FILEcopy :" + item.archivos[0].fileCopyUri);
                                const url = decodeURI(item.archivos[0].fileCopyUri);
            ...
            return (
                <><View style={styles.ctnDefault}>
                    <Text style={styles.textTotal}>No tienes artículos en el carrito!!</Text>
                </View></>)
        }
    }
